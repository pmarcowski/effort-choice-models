...
PAPER_CONFIGS = {
    'Increasing': {'δ₁': -12.595, 'γ₁': 0.6043103, 'δ₂': 0.4693622, 'γ₂': 0.469386, 'ω': 0.1462272},
    'Decreasing-Increasing': {'δ₁': -9.522435, 'γ₁': 10.58172, 'δ₂': 1.901854, 'γ₂': 1.491141, 'ω': 0.4928382},
    'Decreasing': {'δ₁': -1e-07, 'γ₁': 9.582383e-07, 'δ₂': 4.55966, 'γ₂': 3.306229, 'ω': 1e-07},
    'Increasing-Decreasing': {'δ₁': -3.492561, 'γ₁': 2.43447, 'δ₂': 3.258896, 'γ₂': 25.08094, 'ω': 0.382991}
}
...
def create_parameter_input(param_name, param_label, min_value, max_value, default_value, step):
    control = html.Div(style={'marginBottom': '20px'}, children=[
        html.Label(param_label, style={'fontSize': '18px', 'fontWeight': 'bold', 'display': 'block', 'marginBottom': '5px'}),
        dcc.Input(
...
app.layout = html.Div(style={
    ...
    html.Div(style={'display': 'flex', 'flexDirection': 'row', 'justifyContent': 'center', 'gap': '20px'}, children=[
        # Left: Plot
        html.Div(style={'width': '600px'}, children=[
            dcc.Graph(
                id='sv-plot',
        ...
        html.Div(style={'padding': '0 20px', 'width': '300px'}, children=[
            html.H4('Equation:'),
            html.P("SV = X · [1 - (ω·(δ₁·E^γ₁) + (1-ω)·(δ₂·E^γ₂))]", style={'fontSize': '16px'}),
            html.Div(id='current-params', style={'fontSize': '16px', 'marginBottom': '20px'}),
...
def update_plot_and_values(omega, delta1, gamma1, delta2, gamma2, toggle_state):
    ...
    SV = X * (1 - (omega * (delta1 * E ** gamma1) + (1 - omega) * (delta2 * E ** gamma2)))
    ...
    if not toggle_state:
        ...
        current_params = f"X=1, ω={omega:.3f}, δ₁={delta1:.3f}, γ₁={gamma1:.3f}, δ₂={delta2:.3f}, γ₂={gamma2:.3f}"
        ...
        for idx, (config_name, params) in enumerate(PAPER_CONFIGS.items()):
            ...
            SV_paper = X * (1 - (omega_paper * (delta1_paper * E ** gamma1_paper) + (1 - omega_paper) * (delta2_paper * E ** gamma2_paper)))
            ...
            paper_params_list.append(html.Div([
                html.H5(config_name, style={'marginBottom': '5px', 'marginTop': '15px'}),
                html.P(f"X=1, ω={omega_paper:.3f}, δ₁={delta1_paper:.3f}, γ₁={gamma1_paper:.3f}, δ₂={delta2_paper:.3f}, γ₂={gamma2_paper:.3f}")
            ]))
    ...
    if not toggle_state:
        ...
        system_values = html.Div([
            html.H4('System Weights at E=1:'),
            html.P([
                "Positive System (ω·(δ₁·E^γ₁)): ",
                html.Span(f"{positive_system:.3f}", style={'fontWeight': 'bold'})
            ]),
            html.P([
                "Negative System ((1-ω)·(δ₂·E^γ₂)): ",
                html.Span(f"{negative_system:.3f}", style={'fontWeight': 'bold'})
            ])
        ])
    else:
        system_values = ''
